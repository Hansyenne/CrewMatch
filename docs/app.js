...
function attachSwipeGesture(card,targetId){
  if(!card)return;
  let startX=0,startY=0,currentX=0,dragging=false,moved=false;
  const stage=card.parentElement;
  const setHint=()=>{const ratio=Math.max(-1,Math.min(1,currentX/140));card.style.transform=`translateX(${currentX}px) rotate(${ratio*8}deg)`;card.style.setProperty('--swipe-opacity',Math.min(1,Math.abs(ratio)*1.4));stage.classList.toggle('swiping-left',ratio<-.12);stage.classList.toggle('swiping-right',ratio>.12);};
  const finish=(direction)=>{
    dragging=false;
    stage.classList.remove('swiping-left','swiping-right');
    if(Math.abs(currentX)>110){
      card.classList.add(direction==='like'?'fly-right':'fly-left');
      setTimeout(()=>direction==='like'?handleLike(targetId):handlePass(targetId),180);
    }else{card.style.transition='transform .25s ease';card.style.transform='';setTimeout(()=>card.style.transition='',260);}
    currentX=0;
  };
...
function unmatch(targetId){
 if(!currentUser||!safeId(targetId)||targetId===currentUser.id)return;
 const db=getDB();
 const target=findUser(db,targetId);
 if(!target)return;
 const isMutual=mutual(db,currentUser.id,targetId);
 if(!isMutual)return;
 if(!confirm(`Unmatch with ${target.name}? This will remove the connection.`))return;
 db.matches=db.matches.filter(m=>!((m.user1===currentUser.id&&m.user2===targetId)||(m.user1===targetId&&m.user2===currentUser.id)));
 db.likes=db.likes.filter(x=>!((x.from===currentUser.id&&x.to===targetId)||(x.from===targetId&&x.to===currentUser.id)));
 saveDB(db);
 if(activeChatTargetId===targetId){activeChatTargetId=null;document.getElementById("chatHeadTitle").textContent="Select a conversation";document.getElementById("chatInput").disabled=true;document.getElementById("sendBtn").disabled=true;}
 renderMatches();renderMessagesList();renderActiveMessages();renderDiscoverCard();
 showToast("Connection removed.");
}
window.unmatch=unmatch;
...
function bindDelegation(){
 document.addEventListener("click",e=>{
   ...
   if(a==="pass")handlePass(el.dataset.id);else if(a==="like")handleLike(el.dataset.id);else if(a==="chat")openChat(el.dataset.id);else if(a==="accept")acceptRequest(el.dataset.u1,el.dataset.u2);else if(a==="unmatch")unmatch(el.dataset.id);else if(a==="event")openEventModal(el.dataset.id);
 });
